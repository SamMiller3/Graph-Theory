...
visited_nodes = [[start_node,0]] # 2D array, first column is node, second column is distance from source
...
visited_nodes.append([next_node , min_distance])
...
while len(visited_nodes)!=len(adj_mat):
    ...
    for node in visited_nodes:
        if min_distance > np.min(tent_mat[node[0]]): # check each visited node to see closest adj node
            min_distance = np.min(tent_mat[node[0]]) # store distance
            next_node = np.argmin(tent_mat[node[0]]) # and next node
            prev_node = node[0] # store predecessor
    visited_nodes.append([next_node,min_distance])
    tent_mat[:, next_node] = INF # delete next column
    tent_mat[next_node] += min_distance # update distance
...
print(f"distance is {visited_nodes[target_node][1]}")